...
export function LibraryProvider({ children }) {
  ...
  const isTrackLiked = useCallback((track, artist) => {
    ...
    return (library.likedTracks || []).some(
      (t) => t.track.toLowerCase() === track.toLowerCase() && t.artist.toLowerCase() === artist.toLowerCase()
    );
  }, [library.likedTracks]);

  const isAlbumLiked = useCallback((id, title) => {
    return (library.likedAlbums || []).some(
      (a) => (id && a.id && a.id === id) || (title && a.title && a.title.toLowerCase() === title.toLowerCase())
    );
  }, [library.likedAlbums]);

  const isPlaylistLiked = useCallback((id, title) => {
    return (library.likedPlaylists || []).some(
      (p) => (id && p.id && p.id === id) || (title && p.title && p.title.toLowerCase() === title.toLowerCase())
    );
  ...
  const toggleTrackLike = useCallback(async (track, artist, image = '') => {
    setLibrary((prev) => {
      ...
      if (idx > -1) {
        ...
        list.unshift({ track, artist, image, id: '' });
      }
  ...
  const toggleAlbumLike = useCallback(async (album) => {
    setLibrary((prev) => {
      ...
      const idx = list.findIndex(
        (a) => (album.id && a.id && a.id === album.id) || (a.title && album.title && a.title.toLowerCase() === album.title.toLowerCase())
      );
      if (idx > -1) {
        list.splice(idx, 1);
      } else {
  ...
  const togglePlaylistLike = useCallback(async (playlist) => {
    setLibrary((prev) => {
      ...
      const idx = list.findIndex(
        (p) => (playlist.id && p.id && p.id === playlist.id) || (p.title && playlist.title && p.title.toLowerCase() === playlist.title.toLowerCase())
      );
      if (idx > -1) {
        list.splice(idx, 1);
      } else {
